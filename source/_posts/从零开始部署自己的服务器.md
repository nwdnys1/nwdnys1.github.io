...
3. 每一次部署都需要在虚拟环境下重新安装所有依赖 浪费网络资源 GitHub Actions 提供了缓存指令 来缓存一些经常性不变的资源 可以使用`setup-*`指令来自动配置各个包管理器 包括 npm 和 maven 也可以使用`cache`指令来手动缓存指定的文件 最终的示例代码如下：
