...
- 工作进度：我这几天阅读了黄子昂学长关于 CEDS 的论文 部署了 CEDS 仓库中的代码并进行了简单的测试 大概了解了查询接口和元数据注册接口的使用方式 另外部署了一下张海达学长的导航系统 不过没太看懂怎么使用
...
- 先尝试部署到 jcloud 上 进行查询 然后尝试写一个新的查询最新时间戳的接口 或者使用其他方法
