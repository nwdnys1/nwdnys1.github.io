...
- 并发处理
  ...
  - 中心对于消息的处理使用线程池并发
    - 本来的思路是使用TPE（ThreadPoolExecutor）类初始化线程池 然后用一个ConcurrentHashMap来存储每个请求的线程对象 请求的第一个消息会被线程池随机分配一个线程对象 后续的消息取出map里的线程对象即可保证同一个请求的消息被同一个线程处理 但是我不知道如何使用线程池指定某一个线程处理...
    - 现在的实现比较简单 就是一个List<ExecutorService> 里面存了 n 个单线程池 每次请求根据时间戳取模分配一个线程池处理 不知道可不可以保证均匀

- 态势数据计算部分的架构：

  - 中心提供一个接口`/situation` params 包含经纬度范围和应用的 token
  - 应用发送请求后 中心立马生成一个 requestId（即{timestamp}-{token}）同步响应给应用
  - 中心后台继续异步的进行计算 首先中心会在内存中维护一个 map 名为 situationData 其结构类似于：
    ```
    {
        "requestId": {
            "nodes": {                           # 涉及的节点
    ...
    ```
  - 根据经纬度范围得到涉及的节点数组 并向 situationData 中插入一个新的键值对 随后向消息队列的 `situation/{nodeID}` topic 中放入 payload 为`requestId` 的消息 告知节点开始计算
  - 每个节点从 redis 中取出所有的设备 ID 并通过线程池并发计算每个设备的态势数据 计算过程为：先把设备数据中的时间戳作为态势数据的时间戳放入 然后在本节点中进行范围查询 判断此设备是否需要其他节点的数据 如果需要就发送 http 请求获取 最后进行独占空间的计算 把数据封装好放入中心消息队列的 `situation/center/{requestId}` topic 中 消息格式如下：
    ```
    {
        "nodeId": 1,
        "taxi/0001": {
    ...
    ```
  - 中心收到消息后立马进行合并 根据时间戳判断最新态势数据 如果 `nodes` 中已经收到过此节点的数据 则说明消息重复了 不处理 否则置为 TRUE 然后 cnt-- 当发现所有 node 都已合并（即 cnt==0）将 `done` 字段置为 TRUE 并将态势数据发送到`situation/app/{requestId}`的 topic 中 等应用自己取出
  - timer 类中设置了两个定时 schedule
    - checktimeout 任务会定期查看每个请求的时间戳是否已经过时 2s 如果过时 2s 就删除 如果某个节点的消息出现问题未送达 应用不会一直收不到结果 而是在 2s 超时后收到 error 消息 就可以考虑重发
