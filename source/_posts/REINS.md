...
- 模拟每隔 1s 发送数据 接受到的间隔却会大于 1s
- 开放一下 mysql 的端口
- realtime_map 中接收消息时的 time 和消息中的 time 有延迟 差不多是 2s 不知道为什么 难道消息处理不是实时的？
- 数据压缩成 kfm 说的二进制格式 所有消息和 http 的格式都可以压缩成二进制格式
- 节点刚好处于边界时可能会有点问题 暂时没做处理
- 断开连接时如何告知节点
- 并发处理：节点计算态势数据可以多线程 每一个线程计算一部分设备 向邻居节点发送请求也可以并发处理 另外由于 http 有长连接机制 请求不会重新建立连接
- 中心提供一个接口/situation 应用 A 发送请求后 中心立马响应 并生成一个唯一标识（先用时间戳 如何获取应用的 ID 待定 可以是参数）中心向消息队列中放入 topic 为 situation/{nodeID}的消息 消息中要包含请求 ID 告知节点开始计算 节点计算完后把态势数据放入中心的消息队列 situation/center 的 topic 中 消息中要包含请求 ID、节点 ID、态势数据内容 中心在内存中维护一个 map 键为请求的 ID 值需要包含哪些 node 收到了消息、合并去重后的态势数据、请求的时间戳 收到消息时就立马进行合并 当发现所有 node 都已合并后 放入内存 在前端展示
- 如果发生异常 中心的消息队列会一直缺少某个节点的态势数据 考虑使用定时器进行检查 2s 后返回 error
...
- 下发的态势数据的另一部分 需要把某个范围（可以让设备自己定 比如 100m）内的其他设备以及一些静态障碍物的数据下发给设备 只需要位置即可
- 量化每块空间的价值 在真实场景下做实验 证明价值的有效性以及场景的空间利用率确实提高了
- 还要负责通信协议和数据格式的设计 也就是设备上传数据的格式 以及设备如何和边缘节点建立通信 自己的三次握手四次挥手等

### 11.6 组会

- 设备和节点间的通信可以把 json 压缩为二进制传输
- 态势数据不是下发给设备的

### 11.20 组会

- 每个节点的态势数据汇聚到中心 应用向中心发送请求索要态势数据后 中心异步的向各个节点发送请求获取态势数据 然后异步的放到一个消息队列里 等待应用来取
- range_query 的函数可以优化一下 没必要维护 queryneighbors
