...
- critic 的输入到底应该是状态 s 还是 actor 编码过后的 X'？如果输入的是 X 那么 critic 预测的 reward 就都是一样的了
  1. critic 输入同一批数据时 我们实际希望它能够评估不同 DAG 的质量（也就是 reward）而现在输入同一数据只会得到同一的 reward 无法区分 相对的 原论文输入的是 encode 之后的向量 由于 decoder 比较简单 实际上 enc_i 已经可以确定 DAG 矩阵了 因此我觉得 critic 输入 enc_i 或者是 DAG 矩阵都是可以的 换句话说 critic 就是用来拟合 reward 函数的 所以输入应该和 reward 函数一致
  2. 输出的 reward 都一样 可能也会影响 actor 梯度的计算（因为 reward 是作为基准值进去的） 但我不确定
- 一个小问题：为什么不直接对 reward 使用梯度上升法？是不是因为奖励函数不可导？
- 由于不需要预测长期回报 而是只需要拟合即时奖励 我认为 actor 和 critic 都可以进行预训练（前者先用完整数据集预训练 后者用一些 dag 进行监督学习）以此加快每次训练的速度 论文也提到计算 reward 是耗时的大头 如果可以预训练好一个比较拟合的 critic 就可以省去这部分时间了
- 甚至是不是可以不用 a-c 架构 而是分阶段训练 先用数据集采用 ganblr 生成的大量 dag 训练出 critic 然后用 crtic 训练 actor 呢？如果不行 这个也可以作为对比项来说明 A-C 架构的合理性？如果需要 我觉得我可以来做分离架构的部分 以及预训练的部分
