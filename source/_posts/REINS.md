...
- ganblr 会预先确定 dag 的结构 训练时不会动态改变结构 而且其输入必须要有 label 而且生成的图中 label 是根节点 因此之前希望用 ganblr 生成多样化的 dag 来对 critic 进行训练的方案行不太通 我简单写了 2 种方案
  1. 还是用 dag 为输入 用一些其他库的方法 比如 bnlearn
     - 流程：先采样样本 样本输入 bnlearn 生成一批 dag 输入给 critic 输出预测 reward 然后计算 dag 的真实 reward 计算损失函数
     - 问题：原论文的 critic 先将 enc_i 做平均池化再输入给 mlp 现在 critic 输入改为了 dag 的邻接矩阵 再用这种方式显然不合适 因此我先改用 gnn 来构建 critic 而节点特征可以从前面的 emb 获取 不过我对 gnn 不是很了解 因此不太清楚 gnn 处理这样的 dag 评分效果好不好
  2. 用 enc_i 作为输入 要么输入的是原始数据采样 要么是用 ganblr 生成的相似数据 X'
     ...
     - 问题：我看了一眼架构图 我不太清楚输入 actor 的是相似的数据 还是和原论文一样对列特征的嵌入 这样就和原论文是一样的了（原论文用 tf 的 enc 做嵌入）
     - 如果是嵌入 diffusion 的作用是啥呢？
     - 如果是数据 直接从数据生成 dag 看起来有些不合理 尤其是单层的 decoder 现在 encoder 输出的是 X' 使用这个输入去生成 dag 合适吗？原论文是用 tf 对每个变量进行了特征的提取（或者说压缩）然后再生成 dag 如果 decoder 还是单层的话 现在直接用数据去生成好像有点怪 会不会提取的信息不够充分呢？需要改用 tf 的 decoder 吗？
- reward 函数现在输出的数值都比较接近 可能不利于训练 另外 reward 类初始化的参数先和原论文保持一致 可能需要结合 actor 的配置 而且 lamda 参数是随训练动态变化的 因此 critic 或许只能预训练 而不能完全独立训练（独立训练需要确定合适的惩罚参数）
- CDLR 中其实只是学出了最优的 DAG 但是没有继续进行数据的生成 这个生成过程是需要的 我们应该模仿贝叶斯网络进行数据生成吗？
- load 和 eval 方法需要实现 然后看如何结合 Actor
