...
- 原理部分
  ...
  - GM 如何进行数据的索引？
    ...
    - Z 曲线索引：以经纬度数据为例 通过如图的红色曲线决定了数据的唯一顺序 也就映射到了一维整数上 从而支持 1d 的键值索引 当需要进行 2d 的范围查询时 递归的寻找所有被区域包含的字符串 然后通过二分查找快速找到所有数据 或者对编码作主键建立 B+树索引
...
- 综上 要么基于时间分片使用 GM 要么实现类似于 JUST 的时空索引 是比较合适的方案
...
- 找一找 3d 带时间的数据集
- 这两天需要试一下 3d point hbase 装了很久用不了
- 试一下每个索引工具 支不支持 3d+时间
...
- 读 GM 的源码 一种是读 redis 部分 仿写调用索引和写入存储的接口 另一种是提取出源码建立索引的部分 实现 3d+时间
- 另外 时延比较差 可能需要优化
