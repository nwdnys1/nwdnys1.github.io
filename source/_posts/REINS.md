...
- 是否所有设备发送数据的频率都应该比较高呢？比如有一个设备 30s 才发送一次数据 这 30s 内它就好像失联了一样 没法拿来计算态势数据
- mqtt 判断失去连接是基于心跳机制 也就是说有延迟 存在情况：节点 1 的连接设备列表中有设备 A 但实际上设备 A 已经离开节点 1 的管辖范围 这会导致节点 1 计算其他设备的态势数据时使用了过时的 A 数据 如果所有设备都每隔 1s 发送数据 那么可以把超时判断设为 2s 可以保证延迟不会太高 或者当设备 A 连接节点 2 时 节点 2 广播给附近节点 附近节点就可以删除 A 的数据
- 现在的架构实际上是 一个边缘节点 node0 有一个 mqtt 有一个 clientTracker 连接 根据 topic 前缀（也就是 taxi）创建不同的采集线程 维护这个采集线程所连接的设备 现在只有 taxi 因此也就只有一个采集线程 这个采集线程会不断更新索引定期上传 并存进数据库
- 剩余工作 2 建立心跳机制 原本就有的心跳机制是用于检测采集进程是否可以删除的 就是那个 workers 3 上传数据时更新的索引有问题 解决一下 4 看一下如何下发态势数据 5 模拟不同节点的设备 6 看看相关范围如何维护
- 一条消息不能消费两遍 因此倒时候还是要把新写的部分合并到 sub.py 里
- 模拟发送数据时发现一个问题 模拟每隔 1s 发送数据 接受到的间隔却会大于 1s
- 不知道如何从一个模块取出实体到另一个模块 感觉可以用 redis
