...
- 在某一个节点上计算某一个设备的态势数据时 需要获取一个半径内的其他设备的数据 假设半径为 r 点为 p 我先在本节点的范围内进行范围查询 然后在计算一下 p 点到边界的距离是否小于 r 如果小于 r 就向这个边界的节点发送范围查询(目前使用的是 http 请求) 最后再合并这两个结果 合并去重需要判断时间戳 
- 数据压缩成 kfm 说的二进制格式
- 比较 r 和距离时需要考虑地球是一个球体 如何把经纬度转换为英里？使用 haversine 公式计算这个位置的一英里相当于多少经纬度
- 节点刚好处于边界时可能会有点问题 暂时没做处理
- 态势数据下发给上层应用 我们先做汇聚 理论上跨节点的设备会被两个节点都计算 需要做去重 判断哪个是新的 去重的逻辑如下：每个节点计算出来的态势数据数组通过 http 请求响应给一个上层应用（或者数据中心） 其中态势数据中会包含一个时间戳 这个时间戳是在计算这个设备的态势数据时 把这个设备在 realtimemap 里的 TIMESTAMP 取出来的 用这个来比较最新的态势数据即可
- 断开连接时如何告知节点
...
- range_query的函数可以优化一下 没必要维护queryneighbors