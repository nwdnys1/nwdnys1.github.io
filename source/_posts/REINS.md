...
- 态势数据计算部分的架构：
  ...
  - 中心会在内存中维护一个 map 名为 situationData 其结构类似于：
    ```
    {
        "timestamp": {                            # 计算这次态势数据的时间戳
            "nodes": {                            # 所有节点是否已经收到消息
                1: False,
                2: False,
                ...
    ...
    ```
    还会维护一个 map 维护哪些设备订阅了态势数据 记录他们的设备 id
  - 中心每隔 1s 进行态势数据的计算 先向 situationData 中插入一个新的键值对 随后向消息队列的 `situation/{nodeID}` topic 中放入 payload 为`timestamp` 的消息 告知节点开始计算
  - 每个节点从 redis 中取出所有的设备 ID 并通过线程池并发计算每个设备的态势数据 计算过程为：先把设备数据中的时间戳作为态势数据的时间戳放入 然后在本节点中进行范围查询 判断此设备是否需要其他节点的数据 如果需要就发送 http 请求获取 最后进行独占空间的计算 把数据封装好放入中心消息队列的 `situation/center/{timestamp}` topic 中 消息格式如下：
    ```
    {
        "nodeId": 1,
        "taxi/0001": {
    ...
    ```
  - 中心收到消息后立马进行合并 根据时间戳判断最新态势数据 如果 `nodes` 中已经收到过此节点的数据 则说明消息重复了 不处理 否则置为 TRUE 然后 cnt--
  - 当发现所有 node 都已合并（即 cnt==0）将 `done` 字段置为 TRUE 并将态势数据发送到`situation/app/{deviceID}` topic 中 带上时间戳 设备就可以知道哪个是最新的态势数据了
  - timer 类中设置了两个**定时 schedule**
    - checktimeout 任务会定期查看每个请求的时间戳是否已经过时 2s 如果过时 2s 就删除 如果某个节点的消息出现问题未送达 应用不会一直收不到结果
    - persistData 任务定期将已经完成的请求的态势数据持久化到数据库中 以便后续进行数据分析 这里不做处理直接作为文档存入 MongoDB 中

- 在 flask 里写了一个`/situation`的接口 其会发送请求给中心 并显式 wait 消息队列获得到结果 模拟应用获取态势数据的过程 并计算整个过程的时间 初步测试结果如下：

  - 空转时 RTT 基本上是 0.01-0.02s 左右 中心并发后在 0.05s 左右
  - 10 个设备 基本上 RTT 在 0.05s 以下 中心并发后降到 0.02s 左右 原因未知
  - 查询 9 个节点 共 294 个设备 在 0.2-0.3s 之间 这还是只涉及查询 并未涉及强化学习 中心并发后不变
  - 如果 error 则在 2s 左右（因为 timeout 设置的是 2s）
  - 每隔几秒 RTT 会突然变大 猜测是定时落盘导致的？
  - 上述测试包含网络时延大概在几十 ms 左右
  - 并且这还只是串行的单个请求 高并发的请求可能更离谱 目前采用中心并发处理
  - 而且我看 CPU 占用已经很高了 后续压测估计要减点数据量
  - 改为 push 后 294 个设备态势数据计算一次还是在 0.2s-0.3s 之间 由于现在是 push 模式 这个时间应该是可以接受的
  - 刚重启时空转为 10ms 左右 经过一轮数据后就会变成 50ms 左右 原因未知
...
- push 数据给各个设备 那么设备需要的 region 参数如何获取？
- 以后如果 push 给应用 一个应用可能需要多个设备的数据
- 不知道为什么节点有时候莫名其妙就断开连接了 但是也不报错
...
- 改为 push 因为现在 pull 的模式会导致节点重复计算很多设备数据 而且 pull 的模式就是显式等待节点计算 导致了 RTT 的增加 思路如下：
  - 中心维护一个 situationData 里面存储每个设备最新的态势数据（已去重）
  - 中心每隔 1s 进行任务 向节点发送消息 让节点计算态势数据 节点返回后中心进行去重合并 放入 situationData 中
  - 中心计算完后把态势数据通过消息队列推送给各个设备
  - 如果只是发给每个单独的设备 似乎没有必要汇聚到中心？
